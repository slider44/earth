...
export class UserService {
  ...
  dataChange: BehaviorSubject<EmployeeModel[]> = new BehaviorSubject<
    EmployeeModel[]
  >([]);
  ...
  /** CRUD METHODS */
  getAllUsers(): void {
    this.httpClient.get<EmployeeModel[]>(this.API_URL).subscribe(
      data => {
  ...
  //being used by ngrx/effects
  getAllUsersForStore(): Observable<EmployeeModel[]> {
    return this.httpClient
      .get<EmployeeModel[]>(this.API_URL)
      .pipe(catchError((error: any) => Observable.throw(error.json())));
  }

  // ADD, POST METHOD
  addUser(user: EmployeeModel): void {
    this.httpClient.post(this.API_URL + "/", user).subscribe(
      data => {
  ...
  updateUser(user: EmployeeModel): void {
    this.httpClient.put(this.API_URL + "/" + user._id, user).subscribe(
      data => {
        this.dialogData = user;
  ...
  checkUserExists(user: EmployeeModel): void {
    this.httpClient.get(this.API_URL + "/" + user.firstName).subscribe(
      data => {
